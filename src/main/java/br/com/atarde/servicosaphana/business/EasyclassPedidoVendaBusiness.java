...
public class EasyclassPedidoVendaBusiness extends PedidoVendaBusinessAB {
	...
	public PedidoVendaAB inserir(EasyclassPedidoVenda model) {
		...
		try {
			...
			if (TSUtil.isEmpty(saida)) {
				...
				if (!TSUtil.isEmpty(pedido)) {
					...
					new EasyclassPedidoVendaSapBusinessService().inserir(model);
