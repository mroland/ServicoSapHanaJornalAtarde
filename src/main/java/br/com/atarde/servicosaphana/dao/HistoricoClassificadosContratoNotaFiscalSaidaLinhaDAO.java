...
public class HistoricoClassificadosContratoNotaFiscalSaidaLinhaDAO {
	...
	public void inserirInterface(ClassificadosContratoNotaFiscalSaidaLinha model, TSDataBaseBrokerIf broker) throws TSApplicationException {
		...
		if (!TSUtil.isEmpty(model.getImpostos())) {
			...
			for (ClassificadosContratoNotaFiscalSaidaLinhaImposto imposto : model.getImpostos()) {

				imposto.setLinha(model);

				new HistoricoClassificadosContratoNotaFiscalSaidaLinhaImpostoDAO().inserirInterface(imposto, broker);
