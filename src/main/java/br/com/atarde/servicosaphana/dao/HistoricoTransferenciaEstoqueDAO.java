...
public class HistoricoTransferenciaEstoqueDAO {
	...
	public void inserirInterface(TransferenciaEstoque model, TSDataBaseBrokerIf broker) throws TSApplicationException {
		...
		broker.setSQL("INSERT INTO HISTORICO_TRANSFERENCIA_ESTOQUE(ID, INTERFACE_ORIGINAL_ID, ID_EXTERNO, DATA_DOCUMENTO, DATA_LANCAMENTO, DATA_VENCIMENTO, OBSERVACAO_DIARIO, OBSERVACAO, ESTOQUE_ORIGEM_ID, ESTOQUE_DESTINO_ID, EMPRESA_ID, ORIGEM_ID, FILIAL_ID, DATA_EXPORTACAO, NOTA_FISCAL_SAIDA_REFERENCIA_ID, DEVOLUCAO_NOTA_FISCAL_SAIDA_REFERENCIA_ID, SAP_TRANSFERENCIA_ESTOQUE_ID, DATA_IMPORTACAO, DATA_ATUALIZACAO, STATUS_ID, MENSAGEM_ERRO, ARQUIVO_REMESSA, ARQUIVO_REMESSA_SAP) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);", model.getInterfaceId(), interfaceOriginalId, model.getIdExterno(), model.getDataDocumento(), model.getDataLancamento(), model.getDataVencimento(), model.getObservacaoDiario(), model.getObservacao(), model.getEstoqueOrigem().getId(), model.getEstoqueDestino().getId(), model.getEmpresa().getId(), model.getOrigem().getId(), model.getFilial().getId(), new Timestamp(model.getDataExportacao().getTime()), model.getNotaFiscalSaidaReferenciada().getInterfaceId(), model.getDevolucaoNotaFiscalSaidaReferenciada().getInterfaceId(), model.getId(), TSUtil.isEmpty(model.getDataImportacao()) ? null : new Timestamp(model.getDataImportacao().getTime()), TSUtil.isEmpty(model.getDataAtualizacao()) ? null : new Timestamp(model.getDataAtualizacao().getTime()), model.getStatus().getId(), model.getMensagemErro(), model.getArquivoRemessa(), model.getArquivoRemessaSap());
