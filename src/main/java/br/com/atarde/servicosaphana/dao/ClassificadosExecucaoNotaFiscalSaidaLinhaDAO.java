...
public class ClassificadosExecucaoNotaFiscalSaidaLinhaDAO {
	...
	public List<ClassificadosExecucaoNotaFiscalSaidaLinha> pesquisarInterface(ClassificadosExecucaoNotaFiscalSaida model) {
		...
		broker.setSQL("SELECT ID , ITEM , QUANTIDADE , VALOR , CODIGO_IMPOSTO , CLASSIFICADOS_EXECUCAO_NFF_SAIDA_ID , UTILIZACAO_ID , DEPOSITO_ID, UNIDADE_NEGOCIO_ID, CONTA_CONTABIL_ID, PROJETO_ID FROM CLASSIFICADOS_EXECUCAO_NFF_SAIDA_LINHA S WHERE S.CLASSIFICADOS_EXECUCAO_NFF_SAIDA_ID = ?", model.getInterfaceId());

		return broker.getCollectionBean(ClassificadosExecucaoNotaFiscalSaidaLinha.class, "interfaceId", "item.id", "quantidade", "valor", "codigoImposto.id", "notaFiscalSaida.id", "utilizacao.id", "estoque.id", "unidadeNegocio.id", "contaContabil.id", "projeto.id");
	}
