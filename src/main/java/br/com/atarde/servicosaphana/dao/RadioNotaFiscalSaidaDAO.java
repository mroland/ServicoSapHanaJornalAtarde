...
public class RadioNotaFiscalSaidaDAO {
	...
	public void alterarInterface(RadioNotaFiscalSaida model) throws TSApplicationException {
		...
		broker.setSQL("UPDATE RADIO_NFF_SAIDA SET STATUS_ID = ?, MENSAGEM_ERRO = ?, DATA_ATUALIZACAO =?, DATA_IMPORTACAO = ? WHERE ID = ?", model.getStatus().getId(), model.getMensagemErro(), model.getDataAtualizacao(), model.getDataImportacao(), model.getInterfaceId());
