	...
	public ClassificadosExecucaoNotaFiscalSaida inserirInterface(ClassificadosExecucaoNotaFiscalSaida model) throws TSApplicationException {
		...
		if (!TSUtil.isEmpty(model.getParcelas())) {
			...
			for (ParcelaAB item : model.getParcelas()) {
				...
				ClassificadosExecucaoNotaFiscalSaidaParcela linha = (ClassificadosExecucaoNotaFiscalSaidaParcela) item;

				linha.setNotaFiscalSaida(new ClassificadosExecucaoNotaFiscalSaida("interfaceId", model.getInterfaceId()));

				new ClassificadosExecucaoNotaFiscalSaidaParcelaDAO().inserirInterface(linha, broker);
