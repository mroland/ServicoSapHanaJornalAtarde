...
public class EasyclassPedidoVendaDAO {
	...
	public void alterarInterface(EasyclassPedidoVenda model) throws TSApplicationException {
		...
		broker.setSQL("UPDATE EASYCLASS_PEDIDOVENDA SET STATUS_ID = ?, MENSAGEM_ERRO = ?, DATA_ATUALIZACAO =?, DATA_IMPORTACAO = ? WHERE ID = ?", model.getStatus().getId(), model.getMensagemErro(), TSUtil.isEmpty(model.getDataAtualizacao()) ? null : new Timestamp(model.getDataAtualizacao().getTime()), TSUtil.isEmpty(model.getDataImportacao()) ? null : new Timestamp(model.getDataImportacao().getTime()), model.getInterfaceId());
	...
	public void excluirInterface(EasyclassPedidoVenda model) throws TSApplicationException {
		...
		broker.setSQL("DELETE FROM EASYCLASS_PEDIDOVENDA WHERE ID = ?", model.getInterfaceId());
	...
	public List<EasyclassPedidoVenda> pesquisarPorAtrasadaInterface(EasyclassPedidoVenda model) {
		...
		broker.setSQL("SELECT ID FROM EASYCLASS_PEDIDOVENDA A WHERE STATUS_ID = ? AND (DATE_PART('day', NOW()::timestamp - DATA_EXPORTACAO::timestamp) * 24 + DATE_PART('hour', NOW()::timestamp - DATA_EXPORTACAO::timestamp)) * 60 + DATE_PART('minute', NOW()::timestamp - DATA_EXPORTACAO::timestamp) >=30", model.getStatus().getId());

		return broker.getCollectionBean(EasyclassPedidoVenda.class, "interfaceId");

	}

	public EasyclassPedidoVenda obterIdExternoInterface(EasyclassPedidoVenda model) {

		TSDataBaseBrokerIf broker = TSDataBaseBrokerFactory.getDataBaseBrokerIf();

		broker.setSQL("SELECT ID FROM EASYCLASS_PEDIDOVENDA A WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresa().getId());
