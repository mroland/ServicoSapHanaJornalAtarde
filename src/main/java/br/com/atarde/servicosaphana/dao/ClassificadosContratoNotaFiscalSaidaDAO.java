...
public class ClassificadosContratoNotaFiscalSaidaDAO {
	...
	public void alterarInterface(ClassificadosContratoNotaFiscalSaida model) throws TSApplicationException {
		...
		broker.setSQL("UPDATE CLASSIFICADOS_CONTRATO_NFF_SAIDA SET STATUS_ID = ?, MENSAGEM_ERRO = ?, DATA_ATUALIZACAO =?, DATA_IMPORTACAO = ? WHERE ID = ?", model.getStatus().getId(), model.getMensagemErro(), TSUtil.isEmpty(model.getDataAtualizacao()) ? null : new Timestamp(model.getDataAtualizacao().getTime()), TSUtil.isEmpty(model.getDataImportacao()) ? null : new Timestamp(model.getDataImportacao().getTime()), model.getInterfaceId());
	...
	public void excluirInterface(ClassificadosContratoNotaFiscalSaida model) throws TSApplicationException {
		...
		broker.setSQL("DELETE FROM CLASSIFICADOS_CONTRATO_NFF_SAIDA WHERE ID = ?", model.getInterfaceId());
	...
	public List<ClassificadosContratoNotaFiscalSaida> pesquisarPorAtrasadaInterface(ClassificadosContratoNotaFiscalSaida model) {
		...
		broker.setSQL("SELECT ID FROM CLASSIFICADOS_CONTRATO_NFF_SAIDA WHERE STATUS_ID = ? AND (DATE_PART('day', NOW()::timestamp - DATA_EXPORTACAO::timestamp) * 24 + DATE_PART('hour', NOW()::timestamp - DATA_EXPORTACAO::timestamp)) * 60 + DATE_PART('minute', NOW()::timestamp - DATA_EXPORTACAO::timestamp) >=30", model.getStatus().getId());
	...
	public ClassificadosContratoNotaFiscalSaida obterIdExternoInterface(ClassificadosContratoNotaFiscalSaida model) {
		...
		broker.setSQL("SELECT ID FROM CLASSIFICADOS_CONTRATO_NFF_SAIDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresa().getId());
