...
public class EasyclassNotaFiscalSaidaDAO {
	...
	public List<EasyclassNotaFiscalSaida> pesquisarInterface(EasyclassNotaFiscalSaida model) {
		...
		return broker.getCollectionBean(EasyclassNotaFiscalSaida.class, "interfaceId",
				...
				"vendedor.id", "vendedor.tipoIdentificador", "vendedor.identificador", "vendedor.nome", "vendedor.dataAtualizacao", "vendedor.grupoComissao.id",

				"idExterno", "origem.id", "dataLancamento", "dataDocumento", "dataVencimento", "condicaoPagamento.id", "sequencia.id", "valor", "dataCriacao", "dataAtualizacao", "criadoPor", "atualizadoPor", "dataExportacao", "uValorBruto", "uComissaoAgencia", "uDiasPublicacao", "uDataPublicacaoFinal", "uEnderecoEntrega", "uTituloPublicacao", "status", "uNumeroPI", "uTipoTransacao", "uPeriodo", "uPageViews", "uFormato", "uEntregaVendedor", "uProduto", "uCampanha", "uPostoId", "uDataPublicacaoInicial", "vendedor.uCga", "uObservacao", "uPermuta", "empresa.id", "dataImportacao", "anunciante.enderecoEntregaDefault", "anunciante.enderecoCobrancaDefault", "anunciante.flagEndereco",  "cliente.flagEndereco", "cliente.enderecoEntregaDefault", "cliente.enderecoCobrancaDefault", "filial.id", "uAutorizacaoPublicidade");
	...
	public void alterarInterface(EasyclassNotaFiscalSaida model) throws TSApplicationException {
		...
		broker.setSQL("UPDATE EASYCLASS_NFF_SAIDA SET STATUS_ID = ?, MENSAGEM_ERRO = ?, DATA_ATUALIZACAO =?, DATA_IMPORTACAO = ? WHERE ID = ?", model.getStatus().getId(), model.getMensagemErro(), TSUtil.isEmpty(model.getDataAtualizacao()) ? null : new Timestamp(model.getDataAtualizacao().getTime()), TSUtil.isEmpty(model.getDataImportacao()) ? null : new Timestamp(model.getDataImportacao().getTime()), model.getInterfaceId());
	...
	public void excluirInterface(EasyclassNotaFiscalSaida model) throws TSApplicationException {
		...
		broker.setSQL("DELETE FROM EASYCLASS_NFF_SAIDA WHERE ID = ?", model.getInterfaceId());
	...
	public List<EasyclassNotaFiscalSaida> pesquisarPorAtrasadaInterface(EasyclassNotaFiscalSaida model) {
		...
		broker.setSQL("SELECT ID FROM EASYCLASS_NFF_SAIDA A WHERE STATUS_ID = ? AND (DATE_PART('day', NOW()::timestamp - DATA_EXPORTACAO::timestamp) * 24 + DATE_PART('hour', NOW()::timestamp - DATA_EXPORTACAO::timestamp)) * 60 + DATE_PART('minute', NOW()::timestamp - DATA_EXPORTACAO::timestamp) >=30", model.getStatus().getId());
	...
	public EasyclassNotaFiscalSaida obterIdExternoInterface(EasyclassNotaFiscalSaida model) {
		...
		broker.setSQL("SELECT ID FROM EASYCLASS_NFF_SAIDA A WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresa().getId());
