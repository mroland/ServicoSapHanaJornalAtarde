...
public class HistoricoAssinaturaPedidoVendaLinhaDAO {

	public void inserirInterface(AssinaturaPedidoVendaLinha model, TSDataBaseBrokerIf broker) throws TSApplicationException {

		model.setInterfaceId(broker.getSequenceNextValue("historico_assinaturapedidovenda_linhas_id_seq"));

		broker.setSQL("INSERT INTO HISTORICO_ASSINATURAPEDIDOVENDA_LINHAS(ID, HISTORICO_PEDIDOVENDA_ID, ITEM_ID, QUANTIDADE, VALOR_UNITARIO, VALOR, CODIGO_IMPOSTO_ID, CST_COFINS, CST_ICMS, CST_IPI, CST_PIS, CONTA_CONTABIL_ID, CFOP, CODIGO_BARRAS, UTILIZACAO_ID, VOLUME, FLAG_IMPOSTO, DEPOSITO_ID, UNIDADE_NEGOCIO_ID) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", model.getInterfaceId(), model.getPedidoVenda().getInterfaceId(), 
