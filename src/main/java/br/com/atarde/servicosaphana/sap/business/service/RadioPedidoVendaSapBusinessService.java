...
import br.com.atarde.servicosaphana.sap.hana.model.RadioPedidoVendaLinhaModel;
import br.com.atarde.servicosaphana.sap.hana.model.RadioPedidoVendaModel;
 ...
 * @author mroland
 */
public class RadioPedidoVendaSapBusinessService {
	...
	public RadioPedidoVenda inserir(RadioPedidoVenda model) throws Exception {

		this.initObjetosNaRequisicao(model.getEmpresa());

		RadioPedidoVendaModel nffJson = new RadioPedidoVendaModel();
		...
		//Nao tem sequencia em pedido de venda
		...
		if (TSUtil.isEmpty(nffJson.getLinhas())) {
			...
			nffJson.setLinhas(new ArrayList<RadioPedidoVendaLinhaModel>());

		}

		RadioPedidoVendaLinhaModel linhaJson;
		for (RadioPedidoVendaLinha linha : model.getLinhas()) {

			linhaJson = new RadioPedidoVendaLinhaModel();
	...
	private RadioPedidoVendaModel inserir(RadioPedidoVendaModel model, ConexaoSessaoHanaModel conexaoSessaoHanaModel) throws Exception {

		String arquivoRemessaSap = new Gson().toJson(model);
		// System.out.println(jsonRemessaSap);
		...
		RadioPedidoVendaModel resposta;
		...
		if (response.getStatusInfo().getStatusCode() == 200 || response.getStatusInfo().getStatusCode() == 201) {
			...
			resposta = new Gson().fromJson(json, RadioPedidoVendaModel.class);
