 ...
 */
@WebServlet("/ClassificadosExecucaoNotaFiscalSaidaServlet")
public class ClassificadosExecucaoNotaFiscalSaidaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ClassificadosExecucaoNotaFiscalSaidaServlet() {
		super();
	...
	private void processar(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (TSUtil.isEmpty(empresaId)) {
			...
			if (TSUtil.isEmpty(empresa)) {
				...
				new ClassificadosExecucaoNotaFiscalSaidaBusiness().inserirSAP(empresa);
