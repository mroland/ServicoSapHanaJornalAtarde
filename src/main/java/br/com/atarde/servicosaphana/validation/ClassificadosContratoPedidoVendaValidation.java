...
public class ClassificadosContratoPedidoVendaValidation extends PedidoVendaValidation {

	public String validar(ClassificadosContratoPedidoVenda model) {
	...
	public String validaNFF(PedidoVendaAB model) {

		StringBuilder retorno = new StringBuilder();

		if (TSUtil.isEmpty(retorno.toString())) {

			ClassificadosContratoPedidoVenda nota = (ClassificadosContratoPedidoVenda) model;

			if (!TSUtil.isEmpty(new ClassificadosContratoPedidoVendaDAO().obterIdExternoInterface(nota)) || this.isExisteNotaFiscalSaidaInterface(nota)) {
			...
			if (!TSUtil.isEmpty(nota.getLinhas()) || (!TSUtil.isEmpty(nota.getLinhas()) && nota.getLinhas().size() != 0)) {
				...
				for (ClassificadosContratoPedidoVendaLinha linha : nota.getLinhas()) {
	...
	private boolean isExisteNotaFiscalSaidaInterface(ClassificadosContratoPedidoVenda model) {

		boolean retorno = false;

		ClassificadosContratoNotaFiscalSaida saida = new ClassificadosContratoNotaFiscalSaida(model.getEmpresa());
		saida.setOrigem(model.getOrigem());
		saida.setIdExterno(model.getIdExterno());

		if (!TSUtil.isEmpty(new ClassificadosContratoNotaFiscalSaidaDAO().obterIdExternoInterface(saida))) {
	...
	protected String validaLinhaNFF(ClassificadosContratoPedidoVendaLinha model, Filial filial) {
