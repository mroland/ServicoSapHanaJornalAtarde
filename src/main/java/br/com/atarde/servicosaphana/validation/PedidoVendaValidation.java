...
public class PedidoVendaValidation extends DocumentoValidationAB {

	public String validar(PedidoVendaAB model) {
		...
		if (TSUtil.isEmpty(model)) {
			...
			if (TSUtil.isEmpty(model.getEmpresa()) || (TSUtil.isEmpty(new EmpresaDAO().obter(model.getEmpresa())))) {
				...
				retorno.append(this.validaPedidoVenda(model));
	...
	private String validaPedidoVenda(PedidoVendaAB model) {
		...
		if(TSUtil.isEmpty(model.getIdExterno())){
			...
			if (!TSUtil.isEmpty(new PedidoVendaDAO().obterIdExterno(model)) || this.isExisteNotaFiscal(model)) {
	...
	private boolean isExisteNotaFiscal(PedidoVendaAB model) {
		...
		if (!TSUtil.isEmpty(new NotaFiscalSaidaDAO().obterIdExterno(saida))) {
