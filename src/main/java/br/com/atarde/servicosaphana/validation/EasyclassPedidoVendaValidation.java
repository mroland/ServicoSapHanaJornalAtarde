...
public class EasyclassPedidoVendaValidation extends PedidoVendaValidation {

	public String validar(EasyclassPedidoVenda model) {
	...
	public String validaNFF(PedidoVendaAB model) {
		...
		if (TSUtil.isEmpty(retorno.toString())) {
			...
			EasyclassPedidoVenda nota = (EasyclassPedidoVenda) model;
			
			if(!TSUtil.isEmpty(new EasyclassPedidoVendaDAO().obterIdExternoInterface(nota)) || this.isExisteNotaFiscalSaidaInterface(nota)) {
			...
			if (!TSUtil.isEmpty(nota.getLinhas()) || (!TSUtil.isEmpty(nota.getLinhas()) && nota.getLinhas().size() != 0)) {
				...
				for (EasyclassPedidoVendaLinha linha : nota.getLinhas()) {

					linha.setEmpresa(model.getEmpresa());

					retorno.append(this.validaLinhaPedidoVenda(linha, model.getFilial()));
	...
	private boolean isExisteNotaFiscalSaidaInterface(EasyclassPedidoVenda model) {

		boolean retorno = false;

		EasyclassNotaFiscalSaida saida = new EasyclassNotaFiscalSaida(model.getEmpresa());
		saida.setOrigem(model.getOrigem());
		saida.setIdExterno(model.getIdExterno());

		if (!TSUtil.isEmpty(new EasyclassNotaFiscalSaidaDAO().obterIdExternoInterface(saida))) {
	...
	protected String validaLinhaPedidoVenda(EasyclassPedidoVendaLinha model, Filial filial) {
